...
const SuiviAbsencesEnfant = () => {
    const { utilisateur, donnees } = useAuth();
    const [filtreEnfant, setFiltreEnfant] = useState(''); // Filtre pour sélectionner un enfant
    ...
    const mesEnfants = utilisateur?.enfants || []; // Les enfants directement associés au parent
    const matieres = donnees.matieres || [];
    const enseignants = donnees.enseignants || [];

    // Sélectionner l'élève dont les absences sont affichées (pour le titre)
    const enfantActuel = useMemo(() => {
        if (filtreEnfant) {
            return mesEnfants.find(e => String(e.id) === filtreEnfant);
        }
        // Par défaut, si aucun enfant n'est filtré, ou si un seul enfant, on peut l'afficher
        return mesEnfants.length === 1 ? mesEnfants[0] : null;
    }, [filtreEnfant, mesEnfants]);

    // Filtrer les présences pour l'enfant sélectionné (ou tous si pas de filtre)
    const presencesDeLenfant = useMemo(() => {
        if (!mesEnfants.length) return []; // Si le parent n'a pas d'enfants
        
        const idsEnfantsFiltres = filtreEnfant ? [Number(filtreEnfant)] : mesEnfants.map(e => e.id);

        return toutesLesPresences.filter(p => idsEnfantsFiltres.includes(p.eleveId));
    }, [toutesLesPresences, mesEnfants, filtreEnfant]);

    const presencesFiltrees = useMemo(() => {
        return presencesDeLenfant.filter(presence => {
    ...
    }, [presencesDeLenfant, filtreType, filtreJustifie, dateDebut, dateFin]);

    const calculerStatistiques = useMemo(() => {
        const totalEvenementsNonPresents = presencesFiltrees.filter(p => p.type !== 'Présent').length;
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">
                        ...
                        {enfantActuel && (
                            <span className="ml-2 text-fleuve-700">
                                - {enfantActuel.prenom} {enfantActuel.nom}
                            </span>
                        )}
                    </h1>
                    <p className="text-gray-600">Consultez l'historique de présence de vos enfants</p>
                </div>
                    ...
                    <tbody className="table-body">
                        {presencesFiltrees.length > 0 ? (
                            presencesFiltrees.map(presence => {
                                const enfant = mesEnfants.find(e => e.id === presence.eleveId);
...
export default SuiviAbsencesEnfant;