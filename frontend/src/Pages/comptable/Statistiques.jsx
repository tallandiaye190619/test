...
const StatistiquesFinancieres = () => {
  ...
  const obtenirEvolutionMensuelle = () => {
    ...
    const startYear = Math.min(...paiements.map(p => new Date(p.datePayment).getFullYear()), currentYear - 2); // Historique sur 3 ans minimum

    const yearsToDisplay = Array.from({ length: currentYear - startYear + 1 }, (_, i) => startYear + i);
    ...
    // Filtrer pour n'afficher que l'année sélectionnée dans le graphique si la période est "mois"
    if (periodeSelectionnee === 'mois' || periodeSelectionnee === 'annee') {
      return evolution.filter(data => data.year === anneeSelectionnee);
    }
    return evolution;
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="card bg-terre-50 border border-terre-200 p-4 rounded-lg shadow-md transition-all duration-200 hover:shadow-lg">
          <div className="flex items-center justify-between">
            ...
            <DollarSign className="h-12 w-12 text-terre-600" /> {/* Remplacer par DollarSign ou autre icône plus pertinente */}
          </div>
        </div>
      </div>
      ...
      <div className="card p-6 shadow-sm">
        ...
        <div className="overflow-x-auto">
          <div className="flex space-x-4 pb-4 items-end" style={{ minWidth: '1000px' }}> {/* Adjusted minWidth and items-end */}
            {evolutionMensuelle.length > 0 ? (
                ...
                <div key={index} className="flex-1 min-w-[60px] text-center">
                    ...
                    <div className="relative w-full bg-gray-200 rounded-lg overflow-hidden h-32 flex items-end"> {/* Fixed height for bars container */}
                        <div
                            className="w-full rounded-t-lg transition-all duration-300 ease-in-out bg-fleuve-500 hover:bg-fleuve-600" // Couleur fleuve
                            style={{ 
                                height: `${Math.max((data.montant / maxMontantMensuel) * 100, 2)}%`, // Minimum 2% pour visibilité
                            }}
                ...
                .map(([type, stats]) => {
                  ...
                  return (
                    <div key={type} className="space-y-2">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2.5"> {/* Barre de progression plus épaisse */}
                        <div 
                          className="bg-fleuve-600 h-2.5 rounded-full transition-all duration-300" // Couleur fleuve
          ...
          <div className="space-y-4">
            {Object.entries(statsParMethode).length > 0 ? (
              Object.entries(statsParMethode)
                ...
                .map(([methode, stats]) => {
                  const pourcentage = (stats.montant / statsGenerales.totalCollecte) * 100;
                  // Assurez-vous que ces couleurs existent dans votre thème ou utilisez des génériques
                  const couleurMethode = {
                      'Orange Money': 'bg-soleil-500', // Soleil pour Orange Money
                      'Wave': 'bg-fleuve-500',      // Fleuve pour Wave
                      'Espèces': 'bg-acacia-500',   // Acacia pour Espèces
                      'Free Money': 'bg-terre-500', // Terre pour Free Money
