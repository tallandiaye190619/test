...
const NotificationsEnseignant = () => {
  ...
  const marquerCommeLue = (id) => {
    ...
    // Implémenter la logique de mise à jour de l'état ou de l'API ici
  };
  ...
  const obtenirIconePriorite = (priorite) => {
    switch (priorite) {
      case 'haute':
        return <AlertCircle className="h-5 w-5 text-soleil-500" />; // Icône AlertCircle pour haute
      case 'urgente':
        return <AlertCircle className="h-5 w-5 text-terre-500" />; // Icône AlertCircle pour urgente
      default:
        return <Info className="h-5 w-5 text-fleuve-500" />; // Icône Info pour normale
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="card p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
          <Filter className="h-6 w-6 text-gray-500 flex-shrink-0" /> {/* Icône plus grande */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 w-full"> {/* Ajusté pour mobile */}
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2"> {/* Espacement ajusté */}
                    {obtenirIconePriorite(notification.priorite)}
                    <h3 className={`text-lg font-bold ${ // Font-bold pour les titres
                      !notification.lue ? 'text-gray-900' : 'text-gray-700' // Texte plus foncé si lu
                    }`}>
                      {notification.titre}
                    </h3>
                    {!notification.lue && (
                  ...
                  <p className={`mb-3 ml-8 ${ // Marge pour aligner avec le titre
                    !notification.lue ? 'text-gray-800' : 'text-gray-600' // Texte plus foncé si lu
                  } line-clamp-3`}> {/* Limite de ligne pour le message */}
                    {notification.message}
                  </p>
                  
                  <div className="flex flex-wrap items-center space-x-4 text-sm text-gray-500 ml-8 mt-4"> {/* Marge et flex-wrap */}
                    <span className="flex items-center"><UserCircle className="h-4 w-4 mr-1" /> De: {notification.expediteur}</span>
                    <span>•</span>
                    <span className="flex items-center"><Calendar className="h-4 w-4 mr-1" /> {new Date(notification.date).toLocaleDateString('fr-FR')}</span>
                    <span>•</span>
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${
                      ...
                      {notification.priorite.charAt(0).toUpperCase() + notification.priorite.slice(1)} {/* Capitalize */}
                    </span>
                  </div>
                </div>
                
                <div className="flex flex-col items-center space-y-2 ml-4 flex-shrink-0"> {/* Utilisation de flex-col pour les boutons */}
                  {!notification.lue ? (
                    <button
                    ...
                    >
                      <CheckCircle className="h-5 w-5" /> {/* Icône plus grande */}
                    </button>
                  ) : (
                    <button
                    ...
                    >
                      <BookmarkCheck className="h-5 w-5" /> {/* Icône plus grande */}
                    </button>
                  )}
                  <button
                    ...
                    title="Supprimer"
                  >
                    <Trash2 className="h-5 w-5" /> {/* Icône plus grande */}
                  </button>
                </div>
              </div>
            </div>
